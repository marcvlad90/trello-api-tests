...
public class BoardApiSteps extends AbstractApiSteps {
    ...
    @Step
    public void populateBoardWithCards() {
        Card cardRequest = CardFactory.getCardInstance(getBoardRandomListId(), cardName);
        Map<String, String> bodyParams = getCommonBodyParams();
    ...
    @Step
    public void createBoardsWithCards(int numberOfBoards) throws IOException {
        for (int i = 0; i < numberOfBoards; i++) {
            createBoard();
            withTestDataFrom(com.tools.constants.Constants.TEST_DATA_FILE_PATH_FOR_CSV + "multipleCards.csv")
            .run(this).populateBoardWithCards();
        }
    }
}
